...
def appender(row, allele_id, all_alleles):
    for allele in alleles:
        if allele_id == allele:
            ...
            return
    print("Unknown allele ID found: ", allele_id)
    sys.exit()
    return
...
def id_writer(all_alleles):
    ids_with_alleles = []
    for element in all_alleles:
        for hit in element:
            ids_with_alleles.append([hit[1], hit[0]])
    with open('filtered_ids.csv', 'w') as writefile:
        csvwriter = csv.writer(writefile, delimiter='\t')
        for element in ids_with_alleles:
            csvwriter.writerow(element)
    return
