...
# Define a set for nodes with distance less than or equal to 2 from any purple node
close_nodes = set()

# For each purple node, we look for nodes with distance 1 and 2 and add them to the set
for node in G.nodes():
    if node_colors[node] == 'purple':
        close_nodes.add(node)
        close_nodes.update(nx.neighbors(G, node))
        for neighbor in list(nx.neighbors(G, node)):
            close_nodes.update(nx.neighbors(G, neighbor))
