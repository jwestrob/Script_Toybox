#If you're looking at this on github: this is NOT MY SCRIPT! It's by Alex CC: https://github.com/alexcritschristoph
import os
import sys
import glob
import pysam
...
def filter_paired_reads(bam, genes, date, min_ani = 0.95, min_mapq = 2):
    '''
    ...
    f = open(bam.split("/")[-1].split(".")[0] + "_" + date + ".counts", 'w+')
    f.write("Scaffold\tORF\tCount\n")
    for scaff in tqdm(genes, desc='Filtering Reads'):
        read_count = 0
        ...
        if read_count > 0:
            try:
                f.write(scaff[0] + "\t" + scaff[3] + "\t" + str(read_count) + "\n")
            except:
                print(scaff)
                sys.exit()
    f.close()
    ...
    print("Reads which also meet min_mapq of " + str(min_mapq) + "\t" + str(mapq_good) + "\t" + str(int(100*float(mapq_good) / total)) +  "%")
    print("Reads which also pass final read pair PID >" + str(min_ani) + "%\t" + str(mm_good) + "\t" + str(int(100*mm_good / total)) + "%")
    samfile.close()
...
def get_genes(genes_file):
    ...
    #    genes.append([line.split()[1],int(line.split()[2]), int(line.split()[3]), line.strip()])
    #f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-date', action='store', nargs=1)
...
#    setup_logger(args.log)
    ...
    filter_paired_reads(args.bam, genes, args.date[0], float(args.mismatch_threshold), int(args.min_mapq))
